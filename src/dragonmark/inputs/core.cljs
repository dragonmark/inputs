...
(defmulti magic-input
          (fn [{opts :opts}]
            (or
              (get-in opts [:$base-opts :shape :fields  (:k opts) :type])
...
(defn button-view
  [app owner {:keys [k comp-name attrs] :as opts}]
  (let [labels (:label opts)]
    (create-component
      {:render
       (fn [this]
  ...
  (let [{:keys [chan lang]} (get-state owner)]
    [:div {:className (styles "form-group" (validation-style val-states))}
     [:label {:htmlFor   (full-name (:k opts))
      ...
      (or
        (get-in opts [:$base-opts :shape :fields (:k opts) :label])
        (i/label opts))]
...
(s/defn
  ...
  ([{:keys [name schema action cancel opts shape] :or {opts {}} :as spec}]
    (make-input-comp name schema action cancel (if shape (assoc opts :shape shape) opts)))
  ([comp-name :- s/Keyword
    schema
    action]
  ...
  ([comp-name :- s/Keyword
    ...
    cancel
    opts :- SchOptions]
    (let [comp-name (or comp-name :form)

          schema (merge schema (-> opts :shape :schema))

          order (or (:order opts) (-> opts :shape :order))
          ext-chan (:ext-chan opts)
          verily-rules (or (:validations opts)
                           (:validation opts)
                           (-> opts :shape :validation))
          ...
          {:get-initial-state
           ...
           (fn [this]
             (binding [*component* this]
               ...
               (let [{:keys [chan action-chan created-chan cancel-chan]} (get-state this)
                 ...
                 (go
                   (when (and (<! cancel-chan) cancel)
                     (js/setTimeout #(dispatch cancel ::cancel))
                     ))
                       ...
                       (if (= :ko v)
                         ...
                         (if (get-in opts [:action :one-shot])
                           (do
                             ...
                             (update-state! this [:action-state :cancel] action-states)
                 ...
                 (go
                   (loop []
                     (when (<! action-chan)
                       (set-state! this [:action-state :cancel] nil)
                   ...
                   [:div {:className (styles "panel panel-default" comp-class)
                    ...
                    [:form {:className "panel-body"
                     ...
                     (into [:div {:className "inputs-group"}]
                           (if order
                             (map (fn [k] (build-input this (assoc (k opts) :k k
                                                                            :$base-opts opts
                                                                            :k-sch
                                                                            (su/get-sch schema k)
                                                                            :i18n (k labels)))) order)
                             (map (fn [[k t]]
                                    (let [k (if (keyword? k) k (:k k))]
                                      (build-input this (assoc (k opts) :k k
                                                                        :$base-opts opts
                                                                        :k-sch t :i18n (k labels))))) schema)))
                        ...
                        [(build-component button-view state this {:state state
                                                                  :opts  {:k         :cancel
                                                                          ...
                                                                          :label     (or
                                                                                       ...
                                                                                       "Cancel")
                                                                          :comp-name comp-name
                                                                          :attrs     (get-in opts [:cancel :attrs])}})]
                        ""
                        )]]]))))})))))
